...
and innovation programme under the Grant Agreement n. 824064
...
\todo{SR can be set to 180, this is in agreement with what is written in the maxSR section 5.1.3. If the service allows that or not is another story. (Idealy we would add the possibility of querying a database by STMOC and getting a response within the STMOC -- once STMOC std, we might want to come back to this idea).}
...
\todo{I added the TIME as an interval of time, consistent with SIA2 and SODA. I personally would have chosen BCRS to be in line with ST-MOCs, but talking to Francois and Pat they wanted this to be inline with SIA2 and SODA. As long as this makes sense and we move fast I am willing to compromise there.}
...
\item[Query by position and time interval]\nolinkurl{http://my.cone/search?RA=10.68\&DEC=41.26\&SR=0.01&TIME=MJD\_MIN MJD\_MAX}
\end{bigdescription}
...
\begin{itemize}
	...
	\item \textbf{Exactly one} \xmlel{FIELD}, representing the right ascension of the source in the ICRS coordinate system, \textbf{must} have \xmlel{ucd}=\texttt{"POS\_EQ\_RA\_MAIN"} \todo{I think this should be pos.eq.ra;meta.main}, and have \xmlel{datatype} set to \texttt{"float"} or \texttt{"double"}.
	\item \textbf{Exactly one} \xmlel{FIELD}, representing the declination of the source in the ICRS coordinate system, \textbf{must} have \xmlel{ucd}=\texttt{"POS\_EQ\_DEC\_MAIN"} \todo{I think this should be pos.eq.dec;meta.main}, and have \xmlel{datatype} set to \texttt{"float"} or \texttt{"double"}.
	\item \textbf{Exactly one} \xmlel{FIELD}, representing the time of the observation, \textbf{must} have \xmlel{ucd}=\texttt{"time.epoch;meta.main"}, and have \xmlel{datatype} set to \texttt{"float"} or \texttt{"double"}. 

\end{itemize}

The VOTable may include an expression of the uncertainty of the positions given in the above mentioned fields to be interpreted either as a positional error of the source positions, or an angular size if the sources are resolved. If this uncertainty is not provided, it should be taken to be zero; otherwise, it may be set for all table entries with a \xmlel{PARAM} in the \xmlel{RESOURCE} which has a UCD that is set to OBS\_ANG-SIZE \todo{what UCD1+ should this be?} and has a value which is the angle in decimal degrees. Alternatively, a different value for each row in the table can be given via a \xmlel{FIELD} in the table having a UCD set to OBS\_ANG-SIZE.
There may be other \xmlel{FIELD}s in the table. Their specification \textbf{should} include description, datatype, and UCD.\todo{A solution to move to UCD1+ would be nice, but it should probably be postponed to a major revision. I think the time is now.}
...
\todo{Do we need to add here something about the fact that sometimes services will provide time and sometimes not?}
...
This is an example of a legal response to a Cone Search query using the time parameter:
\todo{Can you check this please?}
\begin{lstlisting}[language=XML,basicstyle=\footnotesize]
