...
\todo{\ada{For the case in which only information of time is provided or where a cone search is not representing the location of a source (e.g. GW and neutrinos) this can be set to 180 (in agreement with that is written in the maxSR section 5.1.3. If the service allows that or not is another story. (Idealy we would add the possibility of querying a database by STMOC and getting a response within the STMOC -- once STMOC std, come back to this idea?).}}
...
\ada{this is an example of a legal response to A Cone Search query using the time paramter}
...
<FIELD name="dec" datatype="double" unit="degree" ucd="POS_EQ_DEC_MAIN"> \ada{I think this should be
<DESCRIPTION> Declination </DESCRIPTION>
</FIELD>

STILL NEED to add the time 
\end{lstlisting}
