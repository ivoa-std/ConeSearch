...
There \textbf{must} be a single \xmlel{RESOURCE} with \xmlel{type="results"} in the VOTable, and containing a single \xmlel{TABLE}\todo{should we relax on the number of tables?}.
...
\begin{itemize}
	...
	\item \textbf{Exactly one} \xmlel{FIELD}, representing the right ascension of the source in the ICRS coordinate system, \textbf{must} have \xmlel{ucd="POS\_EQ\_RA\_MAIN"}, and have \xmlel{datatype} set to \xmlel{"float"} or \xmlel{"double"}.
	\item \textbf{Exactly one} \xmlel{FIELD}, representing the declination of the source in the ICRS coordinate system, \textbf{must} have \xmlel{ucd="POS\_EQ\_DEC\_MAIN"}, and have \xmlel{datatype} set to \xmlel{"float"} or \xmlel{"double"}.
\end{itemize}
