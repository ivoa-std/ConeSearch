...
A SCS service for accessing catalogue resources is implemented as a synchronous resource that must meet the following requirements:
\begin{itemize}
	\item the resources must respond to requests submitted using an HTTP GET query string as described in Sec.~\ref{sec:2.1};
	\item the response must be a valid VOTable document, providing a table containing the sources of the catalogue whose positions are within the cone described in the query request (see Sec.~\ref{sec:2.2}); or
	\item must return a stubbed VOTable in case of errror, as described in Sec.~\ref{sec:2.3}.
\end{itemize}
